...
import com.datagen.backend.xml.DistributedExecutor;
import com.datagen.backend.xml.FlatExecutor;
...
public class Calculator {
	...
	public static Calculation calculation(List<Schema> schema,long volume,String fileLocation,List<DgenMethod> method,String format,String technique){
		...
		if(format.equals("FLAT")){
			sb = getXmlFlat(schema,method);
		}else if(format.equals("ORIGINAL")){
			sb = getXmlOriginal(schema,method);
		}
	...
	public static String getXmlOriginal(List<Schema> schema,List<DgenMethod> method){
		...
		StringBuilder tempXml= DistributedExecutor.getXmlChunk(schema,valueTotal,currentTotal,maxTotal,method,1,currentValue,tempCurrentValue);
		String sb = tempXml.toString();
		return sb;
	}
	
	public static String getXmlFlat(List<Schema> schema,List<DgenMethod> method){
		List<ValueCheck> valueTotal= FlatOccuranceHelper.setTotalCalculation(schema,method,1);
		List<CurrentValue> currentValue= new ArrayList<CurrentValue>();
		List<CurrentValue> tempCurrentValue= new ArrayList<CurrentValue>();
		StringBuilder tempXml= FlatExecutor.getXmlChunk(schema,valueTotal,method,1,currentValue,tempCurrentValue);
		String sb = tempXml.toString();
		return sb;
	}
